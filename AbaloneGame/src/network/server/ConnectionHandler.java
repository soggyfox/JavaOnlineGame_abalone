...
public class ConnectionHandler implements Runnable {
     ...
     */
    public ConnectionHandler(BufferedReader in, PrintWriter out, MergeServer server) {
        this.in = in;
        this.out = out;
        this.server = server;
    }
    ...
    public void m(String[] args) throws ArgumentException {
        ...
        for (int i = 1; i <= Integer.parseInt(args[0]); i+= 2) {
            if (!Board.isField(args[i] + args[i+1])) {
                throw new ArgumentException(String.format("%s, %s is no valid field on the board.", args[i], args[i+1]));
            }
        }
        // Administration: check the move and apply if it is valid
        String[] from = new String[2 * Integer.parseInt(args[0])];
        System.arraycopy(args, 1, from, 0, from.length);
        Move move = new Move(server.getPlayers().size(), player, Arrays.asList(from),
                Protocol.toVector(args[args.length - 1]));
        if (GameRules.isLegal(move, server.getGame().getBoard())) {
            move.make(server.getGame().getBoard());
            // Response A: broadcast move to all clients and send ProtocolMessages.VALID to client
            send(ProtocolMessages.VALID);
            server.broadcast(ProtocolMessages.MOVE + ProtocolMessages.DELIMITER + Protocol.encodeMove(move));
        } else {
            // Response B: send ProtocolMessages.NOT_VALID to client
            send(ProtocolMessages.NOT_VALID);
        }
    }
